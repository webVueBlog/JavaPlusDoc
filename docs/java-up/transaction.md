...
<!-- TOC -->
  * [开启事务支持](#开启事务支持)
  * [关于 Spring 对事务的支持](#关于-spring-对事务的支持)
  * [事务管理模型](#事务管理模型)
  * [事务传播行为](#事务传播行为)
  * [事务隔离级别](#事务隔离级别)
  * [事务的超时时间](#事务的超时时间)
  * [事务的只读属性](#事务的只读属性)
  * [事务的回滚策略](#事务的回滚策略)
  * [关于 Spring Boot 对事务的支持](#关于-spring-boot-对事务的支持)
<!-- TOC -->
...
Spring 支持两种事务方式，分别是**编程式事务**和**声明式事务**，后者最常见，通常情况下只需要一个 @Transactional 就搞定了（代码侵入性降到了最低），就像这样：
...
1. @Transactional 注解中的 propagation 对应 TransactionDefinition 中的 getPropagationBehavior，默认值为 Propagation.REQUIRED(TransactionDefinition.PROPAGATION_REQUIRED)。
2. @Transactional 注解中的 isolation 对应 TransactionDefinition 中的 getIsolationLevel，默认值为 DEFAULT(TransactionDefinition.ISOLATION_DEFAULT)。
3. @Transactional 注解中的 timeout 对应 TransactionDefinition 中的 getTimeout，默认值为TransactionDefinition.TIMEOUT_DEFAULT。
4. @Transactional 注解中的 readOnly 对应 TransactionDefinition 中的 isReadOnly，默认值为 false。
...
5. ISOLATION_SERIALIZABLE，最高的隔离级别，虽然可以阻止脏读、幻读和不可重复读，但会严重影响程序性能。
