...
只要对数据库稍有了解的人都知道事务具有 ACID 四个基本属性
...
事务就是一系列的操作，要么全部都执行，要都不执行，这其实就是对事务原子性的刻画；虽然事务具有原子性，但是原子性并不是只与事务有关系，它的身影在很多地方都会出现。
...
事务的状态图以及状态的描述取自 Database System Concepts 。
...
『浅入浅出』MySQL 和 InnoDB 就已经介绍过数据库事务的隔离性。
